...
// ============== 配置 ==============
const WATCHLIST = ['HOOD', 'SOUN', 'IONQ', 'DKNG', 'RBLX', 'AFRM', 'UPST', 'HIMS', 'JOBY', 'LUNR', 'CLSK', 'WULF', 'SKLZ', 'QUBT', 'RGTI', 'GRAB'];
const MAX_PRICE = 100;  // 激进型允许更高价位
...
function analyzeStock(data, newsData = null, marketData = null) {
  ...
  if (rsi !== null) {
    if (rsi >= 55 && rsi <= 75) {
      score += 2;  // 强势区间，动量充足
      signals.push(`RSI ${round(rsi, 1)} 强势区间，动量充足 ✓✓`);
    } else if (rsi >= 45 && rsi < 55) {
      score += 1;  // 中性偏强
      signals.push(`RSI ${round(rsi, 1)} 中性偏强 ✓`);
    } else if (rsi > 80) {
      score -= 1;  // 极度超买，即使激进也要回避
      signals.push(`RSI ${round(rsi, 1)} 极度超买，回调风险 ✗`);
    } else if (rsi < 35) {
      score -= 1;  // 太弱，不追
      signals.push(`RSI ${round(rsi, 1)} 太弱，无动量 ✗`);
    } else {
      signals.push(`RSI ${round(rsi, 1)}`);
    }
  }
  ...
  if (sma5 !== null && currentPrice > sma5) {
    score += 1;
    signals.push(`价格 $${round(currentPrice, 2)} > SMA5 $${round(sma5, 2)} 短期强势 ✓`);
  } else if (sma5 !== null) {
    score -= 1;
    signals.push(`价格 < SMA5 $${round(sma5, 2)} 短期走弱 ✗`);
  }

  // 3. 均线多头排列（趋势确认）
  if (sma5 !== null && sma20 !== null && sma5 > sma20) {
    score += 1;
    signals.push(`SMA5 > SMA20 多头排列，趋势向上 ✓`);
  } else if (sma5 !== null && sma20 !== null) {
    signals.push(`SMA5 < SMA20 空头排列 ✗`);
  }
  ...
  if (macd !== null) {
    if (macd.crossover === 'golden') {
      score += 2;
      signals.push(`MACD 金叉，强烈买入 ✓✓`);
    } else if (macd.histogram > 0) {
      score += 1;
      signals.push(`MACD 柱状图为正 (+${macd.histogram})，上升动能 ✓`);
    } else if (macd.crossover === 'death') {
      score -= 2;  // 激进型对死叉更敏感
      signals.push(`MACD 死叉，立即回避 ✗✗`);
    } else {
  ...
  // 5. 当日涨幅（激进型：涨得多才好）
  if (todayChange >= 1.0 && todayChange <= 5.0) {
    score += 1;
    signals.push(`日涨幅 ${round(todayChange, 2)}% 强势上涨 ✓`);
  } else if (todayChange > 5.0 && todayChange <= 10.0) {
    score += 1;  // 激进型不怕追高
    signals.push(`日涨幅 ${round(todayChange, 2)}% 爆发中 ✓`);
  } else if (todayChange > 10.0) {
    signals.push(`日涨幅 ${round(todayChange, 2)}% 过度爆发，追入风险大 ⚠`);
  } else if (todayChange < -1.0) {
    score -= 1;
    signals.push(`日跌幅 ${round(todayChange, 2)}% 弱势，不追 ✗`);
  } else {
    signals.push(`日涨跌 ${round(todayChange, 2)}% 动量不足`);
  }

  // 6. 成交量确认（激进型：放量是关键）
  if (volumeRatio !== null && volumeRatio > 2.0 && todayChange > 0) {
    score += 2;  // 大幅放量上涨，强烈信号
    signals.push(`成交量 ${round(volumeRatio, 1)}x 大幅放量上涨 ✓✓`);
  } else if (volumeRatio !== null && volumeRatio > 1.2 && todayChange > 0) {
    score += 1;
  ...
  // 8. 市场大盘环境 — 不纳入评分（回测验证：对激进型追强策略为负贡献）
  // 大盘数据仍获取并展示，但不影响 score，后续可改为 regime gate / 过滤条件
  ...
  // 固定比例止损止盈（优化后基线：-3% / +8%，与回测对齐）
  const suggestedStop = round(currentPrice * 0.97, 2);
  const suggestedTP = round(currentPrice * 1.08, 2);
