...
// ============== 配置 ==============
const WATCHLIST = ['PFE', 'VZ', 'CSCO', 'BAC', 'USB', 'KEY', 'KO', 'MO', 'GM', 'BMY', 'CMCSA', 'HPQ', 'DOW', 'NEM', 'CLF', 'AA'];
...
function analyzeStock(data, newsData = null, marketData = null) {
  ...
  if (rsi !== null) {
    if (rsi < 25) {
      score += 3;  // 极度超卖，强烈买入信号
      signals.push(`RSI ${round(rsi, 1)} 极度超卖，强反弹信号 ✓✓✓`);
    } else if (rsi < 30) {
      score += 2;  // 超卖区
      signals.push(`RSI ${round(rsi, 1)} 超卖区，反弹概率高 ✓✓`);
    } else if (rsi < 35) {
      score += 1;  // 接近超卖
      signals.push(`RSI ${round(rsi, 1)} 接近超卖区 ✓`);
    } else if (rsi > 60) {
      score -= 1;  // 保守策略不买强势股
      signals.push(`RSI ${round(rsi, 1)} 偏强，不适合超卖反弹 ✗`);
    } else {
      signals.push(`RSI ${round(rsi, 1)} 中性，等待更低位`);
    }
  }

  // 2. 价格低于SMA20（确认超卖，价格偏离均值）
  if (sma20 !== null && currentPrice < sma20) {
    const deviation = ((sma20 - currentPrice) / sma20) * 100;
    if (deviation > 5) {
      score += 2;
      signals.push(`价格偏离SMA20 ${round(deviation, 1)}%，严重超卖 ✓✓`);
    } else {
      score += 1;
      signals.push(`价格 $${round(currentPrice, 2)} < SMA20 $${round(sma20, 2)} 低于均值 ✓`);
    }
  } else if (sma20 !== null) {
    signals.push(`价格 $${round(currentPrice, 2)} > SMA20 $${round(sma20, 2)} 高于均值，不超卖 ✗`);
  }

  // 3. 价格低于SMA5（短期也在跌）
  if (sma5 !== null && currentPrice < sma5) {
    score += 1;
    signals.push(`价格 < SMA5 $${round(sma5, 2)} 短期超卖 ✓`);
  } else if (sma5 !== null) {
    signals.push(`价格 > SMA5 $${round(sma5, 2)} 短期未超卖`);
  }
  ...
  if (macd !== null) {
    if (macd.crossover === 'golden') {
      score += 2;  // 超卖后金叉 = 反弹开始
      signals.push(`MACD 金叉，底部反转信号 ✓✓`);
    } else if (macd.histogram < 0 && macd.histogram > -0.1) {
      score += 1;  // 柱状图接近零轴，可能要翻正
      signals.push(`MACD 柱状图接近零轴 (${macd.histogram})，反转在即 ✓`);
    } else if (macd.histogram < 0) {
      signals.push(`MACD 柱状图为负 (${macd.histogram})，仍在下跌`);
    } else {
  ...
  // 5. 当日跌幅（保守型：跌得多才是机会）
  if (todayChange <= -2.0 && todayChange >= -5.0) {
    score += 1;
    signals.push(`日跌幅 ${round(todayChange, 2)}% 恐慌抛售，反弹机会 ✓`);
  } else if (todayChange < -5.0) {
    signals.push(`日跌幅 ${round(todayChange, 2)}% 暴跌，等待企稳 ⚠`);
  } else if (todayChange > 2.0) {
    score -= 1;
    signals.push(`日涨幅 ${round(todayChange, 2)}% 已反弹，错过入场 ✗`);
  } else {
    signals.push(`日涨跌 ${round(todayChange, 2)}%`);
  }

  // 6. 放量下跌后企稳（成交量确认）
  if (volumeRatio !== null && volumeRatio > 1.5 && todayChange < -1) {
    score += 1;
    signals.push(`成交量 ${round(volumeRatio, 1)}x 放量下跌，可能见底 ✓`);
  ...
  // 8. 市场大盘环境 — 不纳入评分（回测验证：对保守型超卖反弹策略为负贡献）
  ...
  // 固定 -8%/+3% 止损止盈，匹配 V1 回测基线参数（不再用 ATR 动态计算）
  const suggestedStop = round(currentPrice * 0.92, 2);
  const suggestedTP = round(currentPrice * 1.03, 2);
